...
import { View, Text, Pressable, TextInput, StyleSheet, Alert, ScrollView, Dimensions, Platform } from 'react-native';
...
import { CARD_WIDTH, CARD_HEIGHT, calculateFontSize } from '../utils/cardDimensions';
...
const predefinedColors = [
  ...
  "#f38ba8", "#a8e6cf", "#dda0dd", "#98d8c8", "#f7dc6f", "#bb8fce",
  "#85c1e9", "#f8c471", "#82e0aa", "#f1948a", "#85929e", "#d5dbdb",
  
  // Nuances de gris
  "#f8f9fa", "#e9ecef", "#dee2e6", "#ced4da", "#adb5bd", "#6c757d",
  "#495057", "#343a40", "#212529", "#1a1a1a", "#0f0f0f", "#050505"
];
...
export default function CardEditorScreenSimple({ navigation, route }) {
  ...
  const getNumericIndex = (elementIndexString) => {
    ...
    if (typeof elementIndexString === 'string' && elementIndexString.includes('-')) {
  ...
  const updateCardElements = (newElements) => {
    ...
    console.log('✅ currentCard mis à jour:', {
      cardName: updatedCard.name,
      elementsCount: updatedCard.elements?.length,
  ...
  const addElement = (type) => {
    ...
    console.log('✅ Élément ajouté:', { type, newElement, totalElements: updatedElements.length });
  };
  ...
  const startTextEditing = (elementIndex) => {
    ...
    const index = typeof elementIndex === 'string' ? 
      parseInt(elementIndex.split('-')[1] || '0') : elementIndex;
  ...
  const renderPropertyPanel = () => {
    ...
    return (
      <ScrollView style={styles.propertyContent} showsVerticalScrollIndicator={false}>
        ...
        {element.type === 'text' && (
          <>
            ...
            <View style={styles.fontSizeControl}>
              <Text style={styles.propertyLabel}>Taille de police: {element.style?.fontSize || 16}px</Text>
              <View style={styles.fontSizeButtons}>
                <Pressable
                  style={styles.fontSizeButton}
                  onPress={() => updateElementStyle(numericIndex, { 
                    fontSize: Math.max(8, (element.style?.fontSize || 16) - 1) 
                  })}
                >
                  <MaterialCommunityIcons name="minus" size={16} color="#007AFF" />
                </Pressable>
                <Text style={styles.fontSizeValue}>{element.style?.fontSize || 16}px</Text>
                <Pressable
                  style={styles.fontSizeButton}
                  onPress={() => updateElementStyle(numericIndex, { 
                    fontSize: Math.min(72, (element.style?.fontSize || 16) + 1) 
                  })}
                >
            ...
            <View style={styles.fontFamilyControl}>
              ...
              <View style={styles.fontFamilyButtons}>
                {[
                  ...
                  { name: 'San Francisco', family: Platform.OS === 'ios' ? 'San Francisco' : 'Roboto' },
                  { name: 'Helvetica', family: Platform.OS === 'ios' ? 'Helvetica' : 'sans-serif' },
                  { name: 'Monospace', family: Platform.OS === 'ios' ? 'Courier' : 'monospace' }
                ].map(font => (
                  <Pressable
                    key={font.name}
                    style={[
                      styles.fontFamilyButton,
                      element.style?.fontFamily === font.family && styles.selectedFontButton
                    ]}
                    onPress={() => updateElementStyle(numericIndex, { fontFamily: font.family })}
                  >
                    <Text style={[styles.fontButtonText, { fontFamily: font.family }]}>{font.name}</Text>
                  </Pressable>
                ))}
              </View>
            </View>
            ...
            <View style={styles.fontStyleControl}>
              ...
              <View style={styles.fontStyleButtons}>
                <Pressable
                  style={[
                    styles.styleButton,
                    element.style?.fontWeight === 'bold' && styles.selectedStyleButton
                  ]}
                  onPress={() => updateElementStyle(selectedElementIndex, { 
                    fontWeight: element.style?.fontWeight === 'bold' ? 'normal' : 'bold' 
                  })}
                >
                  <MaterialCommunityIcons name="format-bold" size={20} color="#007AFF" />
                </Pressable>
                <Pressable
                  style={[
                    styles.styleButton,
                    element.style?.fontStyle === 'italic' && styles.selectedStyleButton
                  ]}
                  onPress={() => updateElementStyle(selectedElementIndex, { 
                    fontStyle: element.style?.fontStyle === 'italic' ? 'normal' : 'italic' 
                  })}
                >
                  <MaterialCommunityIcons name="format-italic" size={20} color="#007AFF" />
                </Pressable>
                <Pressable
                  style={[
                    styles.styleButton,
                    element.style?.textDecoration === 'underline' && styles.selectedStyleButton
                  ]}
                  onPress={() => updateElementStyle(selectedElementIndex, { 
                    textDecoration: element.style?.textDecoration === 'underline' ? 'none' : 'underline' 
                  })}
                >
                  <MaterialCommunityIcons name="format-underline" size={20} color="#007AFF" />
                </Pressable>
              </View>
            </View>
              ...
              <View style={styles.textAlignButtons}>
                {[
                  ...
                  <Pressable
                    ...
                    style={[
                      styles.alignButton,
                      element.style?.textAlign === align && styles.selectedAlignButton
                    ]}
                    onPress={() => updateElementStyle(selectedElementIndex, { textAlign: align })}
                  >
                ...
                {predefinedColors.map(color => (
                  <Pressable
                    ...
                    style={[
                      ...
                      element.style?.color === color && styles.selectedColorOption
                    ]}
                    onPress={() => updateElementStyle(selectedElementIndex, { color })}
                  >
            ...
            <View style={styles.backgroundColorPicker}>
              ...
              <View style={styles.colorOptions}>
                <Pressable
                  style={[
                    styles.colorOptionSmall,
                    { backgroundColor: 'transparent', borderWidth: 2, borderColor: '#ddd' },
                    element.style?.backgroundColor === 'transparent' && styles.selectedColorOption
                  ]}
                  onPress={() => updateElementStyle(selectedElementIndex, { backgroundColor: 'transparent' })}
                >
                ...
                {predefinedColors.map(color => (
                  <Pressable
                    ...
                    style={[
                      styles.colorOptionSmall,
                      { backgroundColor: color },
                      element.style?.backgroundColor === color && styles.selectedColorOption
                    ]}
                    onPress={() => updateElementStyle(selectedElementIndex, { backgroundColor: color })}
                  >
        ...
        {element.type === 'image' && (
          <>
            ...
            >
              <MaterialCommunityIcons name="image-plus" size={20} color="#007AFF" />
            ...
            <View style={styles.borderRadiusControl}>
              <Text style={styles.propertyLabel}>Bordure arrondie: {element.style?.borderRadius || 0}%</Text>
              <View style={styles.borderRadiusButtons}>
                <Pressable
                  style={styles.radiusButton}
                  onPress={() => updateElementStyle(selectedElementIndex, { borderRadius: 0 })}
                >
                  <MaterialCommunityIcons name="square-outline" size={20} color="#007AFF" />
                  <Text style={styles.radiusButtonText}>Carré</Text>
                </Pressable>
                <Pressable
                  style={styles.radiusButton}
                  onPress={() => updateElementStyle(selectedElementIndex, { borderRadius: 10 })}
                >
                  <MaterialCommunityIcons name="rounded-corner" size={20} color="#007AFF" />
                  <Text style={styles.radiusButtonText}>Arrondi</Text>
                </Pressable>
                <Pressable
                  style={styles.radiusButton}
                  onPress={() => updateElementStyle(selectedElementIndex, { borderRadius: 50 })}
                >
                  <MaterialCommunityIcons name="circle-outline" size={20} color="#007AFF" />
            ...
            <View style={styles.manualRadiusControl}>
              ...
              <View style={styles.radiusSlider}>
                <Pressable
                  ...
                  onPress={() => updateElementStyle(selectedElementIndex, { 
                    borderRadius: Math.max(0, (element.style?.borderRadius || 0) - 1) 
                  })}
                >
                  <MaterialCommunityIcons name="minus" size={16} color="#007AFF" />
                </Pressable>
                <Text style={styles.radiusValue}>{element.style?.borderRadius || 0}%</Text>
                <Pressable
                  style={styles.radiusSliderButton}
                  onPress={() => updateElementStyle(selectedElementIndex, { 
                    borderRadius: Math.min(50, (element.style?.borderRadius || 0) + 1) 
                  })}
                >
                  <MaterialCommunityIcons name="plus" size={16} color="#007AFF" />
                </Pressable>
        ...
        {element.type === 'line' && (
          <>
            ...
            <View style={styles.lineThicknessControl}>
              <Text style={styles.propertyLabel}>Épaisseur: {element.style?.height || 2}px</Text>
              <View style={styles.thicknessButtons}>
                ...
                <Text style={styles.thicknessText}>{element.style?.height || 2}px</Text>
                <Pressable
                  style={styles.thicknessButton}
                  onPress={() => updateElementStyle(selectedElementIndex, { 
        ...
        <View style={styles.commonControls}>
          ...
          <View style={styles.opacityControl}>
            <Text style={styles.propertyLabel}>Opacité: {Math.round((element.style?.opacity || 1) * 100)}%</Text>
            <View style={styles.opacityButtons}>
              ...
              <Text style={styles.opacityText}>{Math.round((element.style?.opacity || 1) * 100)}%</Text>
              <Pressable
                style={styles.opacityButton}
                onPress={() => updateElementStyle(selectedElementIndex, { 
          ...
          <View style={styles.rotationControl}>
            <Text style={styles.propertyLabel}>Rotation: {element.style?.rotation || 0}°</Text>
            <View style={styles.rotationButtons}>
              ...
              >
                <MaterialCommunityIcons name="rotate-left" size={16} color="#007AFF" />
              </Pressable>
              <Text style={styles.rotationText}>{element.style?.rotation || 0}°</Text>
              <Pressable
                style={styles.rotationButton}
                onPress={() => updateElementStyle(selectedElementIndex, { 
              ...
              >
                <MaterialCommunityIcons name="rotate-right" size={16} color="#007AFF" />
              </Pressable>
            </View>
          </View>
        </View>
  ...
  useEffect(() => {
    const fetchBackgrounds = async () => {
      ...
      try {
        ...
        setAllImages(localBackgrounds.map(url => ({
          type: "image",
          value: url,
          thumb: url
        })));
      ...
      {predefinedColors.map((color, index) => (
        <Pressable
          ...
          style={[
            styles.colorOption,
            { backgroundColor: color },
            selectedBg.type === 'color' && selectedBg.value === color && styles.colorOptionSelected
          ]}
      ...
      {predefinedGradients.map((gradient, index) => (
        <Pressable
          ...
          style={[
            ...
            selectedBg.type === 'gradient' && selectedBg.value === gradient && styles.colorOptionSelected
          ]}
      ...
      {bgLoading ? (
        ...
        allImages.map((img, index) => (
          <Pressable
            ...
            style={[
              styles.imageOption,
              selectedBg.type === 'image' && selectedBg.value === img.value && styles.imageOptionSelected
            ]}
          ...
          >
            ...
            <View style={styles.imagePlaceholder}>
              <MaterialCommunityIcons name="image" size={30} color={colors.mutedText} />
            </View>
          ...
          <View style={styles.cardContainer}>
            {(() => {
              const cardData = {
                ...
                layout: { 
                  background: selectedBg,
                  elements: currentCard?.elements || card?.elements || card?.layout?.elements || []
                },
                backLayout: { 
                  background: selectedBg,
                  elements: []
              ...
              console.log('🚀 CardEditorScreen: Données passées à CardCanvas:', {
                  ...
                  <CardCanvas 
                    ...
                    onElementMove={(elementIndex, percentX, percentY) => {
                      ...
                      if (updatedElements[elementIndex]) {
                        console.log('🔄 Élément trouvé pour déplacement:', updatedElements[elementIndex]);
                        ...
                        console.log('🔄 Élément après déplacement:', updatedElements[elementIndex]);
                        updateCardElements(updatedElements);
                      } else {
                        console.log('❌ Élément non trouvé pour déplacement:', { elementIndex, elementsLength: updatedElements.length });
                      }
                    }}
                    onElementResize={(elementIndex, percentWidth, percentHeight) => {
                      if (!currentCard) return;
                      ...
                      if (updatedElements[elementIndex]) {
                        ...
                        console.log('🔄 Élément trouvé pour redimensionnement:', element);
                        ...
                        if (element.type === 'text') {
                          const newFontSize = calculateFontSizeFromDimensions(percentWidth, percentHeight);
                          newStyle.fontSize = newFontSize;
                          
                          console.log('📝 Ajustement automatique police (comme web):', {
                            percentWidth,
                            percentHeight,
                            newFontSize,
                        ...
                        console.log('🔄 Élément après redimensionnement:', updatedElements[elementIndex]);
                        updateCardElements(updatedElements);
                      } else {
                        console.log('❌ Élément non trouvé pour redimensionnement:', { elementIndex, elementsLength: updatedElements.length });
                      }
                    }}
        ...
        {isEditing && (
          <View style={styles.toolbarSection}>
            ...
            <View style={styles.toolbarContainer}>
              ...
              >
                <MaterialCommunityIcons name="format-text" size={24} color="#007AFF" />
              ...
              >
                <MaterialCommunityIcons name="minus" size={24} color="#007AFF" style={{ transform: [{ rotate: '90deg' }] }} />
              ...
              >
                <MaterialCommunityIcons name="format-align-center" size={24} color="#007AFF" />
              ...
              {selectedElementIndex !== null && (
                <>
                  ...
                  >
                    ...
                    <Text style={[styles.toolButtonText, styles.deleteButtonText]}>Supprimer</Text>
                  </Pressable>
                </>
              )}
            </View>
            ...
            {showAlignmentTools && selectedElementIndex && (
              <View style={styles.alignmentToolbar}>
                ...
                <View style={styles.alignmentGrid}>
                  <Pressable style={styles.alignButton} onPress={() => alignElements('left')}>
                    <MaterialCommunityIcons name="format-align-left" size={20} color="#007AFF" />
                  </Pressable>
                  <Pressable style={styles.alignButton} onPress={() => alignElements('center')}>
                    <MaterialCommunityIcons name="format-align-center" size={20} color="#007AFF" />
                  </Pressable>
                  <Pressable style={styles.alignButton} onPress={() => alignElements('right')}>
                    <MaterialCommunityIcons name="format-align-right" size={20} color="#007AFF" />
                  </Pressable>
                  <Pressable style={styles.alignButton} onPress={() => alignElements('top')}>
                    <MaterialCommunityIcons name="format-align-top" size={20} color="#007AFF" />
                  </Pressable>
                  <Pressable style={styles.alignButton} onPress={() => alignElements('middle')}>
                    <MaterialCommunityIcons name="format-align-middle" size={20} color="#007AFF" />
                  </Pressable>
                  <Pressable style={styles.alignButton} onPress={() => alignElements('bottom')}>
                    <MaterialCommunityIcons name="format-align-bottom" size={20} color="#007AFF" />
                  </Pressable>
                </View>
              </View>
            )}
            ...
            {showPropertyPanel && selectedElementIndex && (
              <View style={styles.propertyPanel}>
                <Text style={styles.subSectionTitle}>Propriétés de l'élément</Text>
                {renderPropertyPanel()}
              </View>
            )}
            ...
            {editingText !== null && (
              <View style={styles.textEditor}>
                ...
                <View style={styles.textEditorButtons}>
                  ...
                  <Pressable style={styles.cancelButton} onPress={cancelTextEdit}>
