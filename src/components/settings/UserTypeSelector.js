...
export default function UserTypeSelector({ user, onSaved }) {
  ...
  useEffect(() => { setValue(user?.userType || ''); }, [user]);
    ...
    <View style={styles.panel}>
      ...
      <Pressable style={styles.select} onPress={() => setOpen(true)}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          {value ? (
            <MaterialCommunityIcons name={(USER_TYPES.find(u => u.key === value)?.icon) || 'account'} size={18} color={colors.text} />
          ) : (
            <MaterialCommunityIcons name={'account-question'} size={18} color={colors.mutedText} />
          )}
          <Text style={[styles.selectText, !value && { color: colors.mutedText }]}>
          ...
          <View style={styles.modalCard}>
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                return (
                  <Pressable onPress={() => { setValue(item.key); setOpen(false); }} style={[styles.optionRow, active && styles.optionRowActive]}>
                    <MaterialCommunityIcons name={item.icon} size={18} color={active ? colors.primary : colors.text} />
                    <Text style={[styles.optionText, active && { color: colors.primary }]}>{item.key}</Text>
                  </Pressable>
                );
              }}
            ...
            <Pressable style={[styles.btn, saving && { opacity: 0.6 }]} onPress={saving ? undefined : onSave}>
              {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>Appliquer</Text>}
            </Pressable>
...
const styles = StyleSheet.create({
  ...
  select: { borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, backgroundColor: '#fff', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  selectText: { color: colors.text, fontWeight: '600' },
  modalBackdrop: { flex: 1, backgroundColor: 'rgba(0,0,0,0.3)', alignItems: 'center', justifyContent: 'center', padding: spacing.md },
  modalCard: { backgroundColor: '#fff', borderRadius: 12, padding: spacing.md, width: '92%' },
  optionRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, paddingHorizontal: 8, borderRadius: 8, backgroundColor: '#fff' },
