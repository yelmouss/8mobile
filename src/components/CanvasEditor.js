...
export default function CanvasEditor({
  ...
  let xPct = (panRefs.current[idx]?.dragging && vis && typeof vis.x === 'number') ? vis.x : xPctBase;
  let yPct = (panRefs.current[idx]?.dragging && vis && typeof vis.y === 'number') ? vis.y : yPctBase;
    // If a resize is active for this element, overlay visual rect
    const visResize = Object.values(handleRefs.current || {}).find((h) => h?.vis && h?.start && h?.start.x !== undefined);
    if (visResize && visResize.vis) {
      // Narrow to current index: search keys matching `${idx}:`
      const anyKey = Object.keys(handleRefs.current || {}).find((k) => k.startsWith(`${idx}:`) && handleRefs.current[k]?.vis);
      if (anyKey) {
  ...
  if (!panRefs.current[idx]) panRefs.current[idx] = { start: { x: xPctBase, y: yPctBase }, pan: null, dragging: false, vis: null };
  ...
  onMoveShouldSetPanResponder: (evt, gesture) => (Math.abs(gesture.dx) > MOVE_TRIGGER_THRESHOLD_PX || Math.abs(gesture.dy) > MOVE_TRIGGER_THRESHOLD_PX),
  onStartShouldSetPanResponderCapture: () => false,
  onMoveShouldSetPanResponderCapture: () => false,
        onPanResponderTerminationRequest: () => false,
        onPanResponderGrant: () => {
          panRefs.current[idx].start = { x: toNum(el?.position?.x, 0), y: toNum(el?.position?.y, 0) };
          ...
          const stPx = panRefs.current[idx].startPx || { left: left || 0, top: top || 0, width: width || 0, height: height || 0 };
          panRefs.current[idx].bounds = {
            minDx: -stPx.left,
            maxDx: (size.w || 0) - (stPx.left + stPx.width),
            minDy: -stPx.top,
            maxDy: (size.h || 0) - (stPx.top + stPx.height),
          };
        ...
        onPanResponderMove: (evt, gesture) => {
          ...
          if (dDx < DRAG_MIN_UPDATE_DELTA_PX && dDy < DRAG_MIN_UPDATE_DELTA_PX) return;
          lastMoveTs.current[key] = now;
          // Clamp to bounds before applying
          const b = panRefs.current[idx]?.bounds || { minDx: -9999, maxDx: 9999, minDy: -9999, maxDy: 9999 };
        ...
        onPanResponderRelease: (evt, gesture) => {
          ...
          const b = panRefs.current[idx].bounds || { minDx: -Infinity, maxDx: Infinity, minDy: -Infinity, maxDy: Infinity };
          ...
          let dx = typeof ldx === 'number' ? ldx : clamp(gesture.dx, b.minDx, b.maxDx);
          let dy = typeof ldy === 'number' ? ldy : clamp(gesture.dy, b.minDy, b.maxDy);
          const sx = panRefs.current[idx].start.x;
          const sy = panRefs.current[idx].start.y;
          let nx = clamp(sx + (dx / (size.w || 1)) * 100, 0, 100 - wPct);
          let ny = clamp(sy + (dy / (size.h || 1)) * 100, 0, 100 - hPct);
  ...
  const ty = panRefs.current[idx].anim.y;
    return (
        <Animated.View
          key={idx}
      style={[styles.abs, { left, top, width, height }, borderStyle, { transform: [{ translateX: tx }, { translateY: ty }] }]}
          onTouchEnd={() => { if (!panRefs.current[idx]?.dragging && !handleActive.current) onSelectIdx(idx); }}
          
          {...(panRefs.current[idx].pan?.panHandlers || {})}
        >
          <View style={[styles.fill, { justifyContent: 'center', alignItems, padding, borderWidth, borderColor, backgroundColor, borderRadius, overflow: 'hidden' }]}> 
          ...
          {isSelected ? (
            <View style={[styles.moveHandle, { left: '50%', marginLeft: -10 }]} {...(panRefs.current[idx].pan?.panHandlers || {})} />
          ) : null}
    ...
    if (el.type === 'image' || el.type === 'socialIcon') {
      ...
      const clipRadius = el?.style?.shape === 'circle' ? minSide / 2 : (brPct ? (brPct / 100) * minSide : 0);
  const tx = panRefs.current[idx].anim.x;
  const ty = panRefs.current[idx].anim.y;
    return (
        <Animated.View
      ...
      style={[styles.abs, { left, top, width, height }, borderStyle, { transform: [{ translateX: tx }, { translateY: ty }] }]}
          ...
          {...(panRefs.current[idx].pan?.panHandlers || {})}
        >
          <View style={[styles.fill, { overflow: 'hidden', borderRadius: clipRadius }]}> 
            {svg ? (
  ...
  const ty = panRefs.current[idx].anim.y;
    return (
        <Animated.View
          key={idx}
      style={[styles.abs, { left, top, width, height }, borderStyle, { justifyContent: 'center', alignItems: 'center' }, { transform: [{ translateX: tx }, { translateY: ty }] }]}
          onTouchEnd={() => { if (!panRefs.current[idx]?.dragging && !handleActive.current) onSelectIdx(idx); }}
  ...
  const sizePx = 20;
    const half = sizePx / 2;
  const common = { position: 'absolute', width: sizePx, height: sizePx, backgroundColor: '#2563eb', borderRadius: sizePx / 2, borderWidth: 2, borderColor: '#fff' };
    const hit = { top: 12, bottom: 12, left: 12, right: 12 };
    // Handle types: n, s, e, w, ne, nw, se, sw
    return (
      <>
    {/* Corners only for size control */}
  <View style={[common, { left: -half, top: -half }]} hitSlop={hit} {...getHandlePan(idx, pct, 'nw', el).panHandlers} />
  <View style={[common, { right: -half, top: -half }]} hitSlop={hit} {...getHandlePan(idx, pct, 'ne', el).panHandlers} />
  <View style={[common, { left: -half, bottom: -half }]} hitSlop={hit} {...getHandlePan(idx, pct, 'sw', el).panHandlers} />
  <View style={[common, { right: -half, bottom: -half }]} hitSlop={hit} {...getHandlePan(idx, pct, 'se', el).panHandlers} />
      </>
    );
  };
  ...
  const getHandlePan = (idx, pct, type, elForHandle) => {
    ...
    if (!handleRefs.current[key]) handleRefs.current[key] = { pan: null, start: { x: pct.xPct, y: pct.yPct, w: pct.wPct, h: pct.hPct }, vis: null };
    ...
    const moveImpl = (st, dxPct, dyPct) => {
      ...
      if (type.includes('w')) { nw = st.w - dxPct; nx = st.x + dxPct; }
      if (type.includes('n')) { nh = st.h - dyPct; ny = st.y + dyPct; }

      // Image: optional circle aspect lock
      if (isImage && lockCircleAspect && (elForHandle?.style?.shape === 'circle')) {
        const anchor = {
          se: { ax: st.x, ay: st.y },
        ...
        nw = size; nh = size;
        if (type === 'se') { nx = anchor.ax; ny = anchor.ay; }
        if (type === 'ne') { nx = anchor.ax; ny = anchor.ay - nh; }
        if (type === 'sw') { nx = anchor.ax - nw; ny = anchor.ay; }
        if (type === 'nw') { nx = anchor.ax - nw; ny = anchor.ay - nh; }
      }
      // Clamp
    ...
    const pan = PanResponder.create({
      ...
      onPanResponderRelease: () => {
        ...
        if (vis) {
          ...
          const cur = arr[idx] || {};
          let sx = vis.x, sy = vis.y, sw = vis.w, sh = vis.h;
          if (SNAP > 0) {
          ...
          arr[idx] = { ...cur, position: { ...(cur.position || {}), x: sx, y: sy }, style: nextStyle };
...
const styles = StyleSheet.create({
  ...
  moveHandle: { position: 'absolute', top: -12, width: 20, height: 20, backgroundColor: '#111827', borderRadius: 10, borderWidth: 2, borderColor: '#fff' },
});
